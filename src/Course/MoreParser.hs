...
tok ::
  Parser a
  -> Parser a
tok a = flbindParser a  (\x ->
  flbindParser spaces (\s -> valueParser x))
...
between ::
  ...
  -> Parser a
  -> Parser a
between l r p = l >>= const p >>= (\v -> r >>= const (valueParser v))
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 p q = p >>= (\c -> list (q >>= const p) >>= (\cs -> valueParser (c:.cs)))
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
-- satisfyAll ps = satisfy (\c -> and (ps <*> c:.Nil))
