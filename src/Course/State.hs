...
findM p (h :. t) = 
  (\b -> if b then pure (Full h) else findM p t) =<< p h
...
firstRepeat ::
  ...
  -> Optional a
firstRepeat l = 
  eval (findM p l) S.empty
    where p a = State (\s -> (S.member a s, S.insert a s))
...
isHappy =
  contains 1 . firstRepeat . produce f
    where 
      f x = toInteger . sum $ map (join (*) . digitToInt) (show' x)
