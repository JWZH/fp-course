...
findLeft f (ListZipper l x r) =
  case break f l of
    (_, Nil) ->  IsNotZ
    (l', h :. t) -> IsZ (ListZipper t h (reverse l' ++ x:.r))
...
findRight f (ListZipper l x r) =
  case break f r of 
    (_, Nil) -> IsNotZ
    (l', h:.t) -> IsZ (ListZipper (reverse l' ++ x:.l)h t)
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight (ListZipper l x (h:.t)) = IsZ (ListZipper (x:.l) h t)
...
moveLeftN n z | n < 0 = moveLeftN (negate n) z
...
moveRightN n z | n < 0 = moveRightN (negate n) z
...
index ::
  ListZipper a
  -> Int
index (ListZipper l _ r) = length l
...
-- /Tip:/ Use `zipWith`
  ListZipper fl fx fr  <*> ListZipper al ax ar =
    ...
    -- f:: a->b->a->b List (a->b) -> List a -> List b
...
instance Extend ListZipper where
  (<<=) f a = ListZipper (unfoldr (f' moveLeft) a) (f a) (unfoldr (f' moveRight) a)
    where f' m z = (\z -> (f z, z)) <$> (toOptional . m) z
